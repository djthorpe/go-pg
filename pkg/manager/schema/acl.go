...
func NewACLItem(v string) (*ACLItem, error) {
	...
	if len(tuples) != 4 {
		return nil, pg.ErrBadParameter.Withf("invalid ACL item: %q", v)
	}
	return &ACLItem{
...
func (acl *ACLItem) UnmarshalJSON(data []byte) error {
	...
	// First, try to unmarshal as a string (text format like "role:priv,priv")
	var str string
	if err := json.Unmarshal(data, &str); err == nil {
		return acl.UnmarshalText([]byte(str))
	}
...
func (a ACLItem) MarshalText() ([]byte, error) {
	...
	if a.Role == "" {
		return nil, httpresponse.ErrBadRequest.With("missing role")
	}
	// Quote role if it contains special characters
	if strings.ContainsAny(a.Role, " :\"',") {
		buf.WriteString(strconv.Quote(a.Role))
	} else {
		buf.WriteString(a.Role)
	}
	buf.WriteRune(':')
	for i, priv := range a.Priv {
		if i > 0 {
			buf.WriteRune(',')
		}
...
// Append an ACLItem to the list. If an item with the same role already exists,
// the privileges are merged.
func (acl *ACLList) Append(item *ACLItem) {
	if acl == nil {
		panic("nil ACLList")
	}

	// If an item with the same role already exists, merge the privileges
	if existing := acl.Find(item.Role); existing != nil {
		for _, priv := range item.Priv {
			if !slices.Contains(existing.Priv, priv) {
				existing.Priv = append(existing.Priv, priv)
			}
		}
		return
	}
